                       ...
                       public TinyGsmHttpsA76xx<TinyGsmSim7672>
{
 ...
 protected:
  ...
  bool getGPSImpl(uint8_t *status,float* lat, float* lon, float* speed = 0, float* course = 0, float* alt = 0,
    ...
    //  3,       15,         01,             00,          20,           28.025728,  S,      153.387835,  E,      230324,  061231.000, -1.1,     0.16,     152.36,    1.90,    1.65,   0.95,     8
    //[<mode>],[<GPS-SVs>],[<GLONASS-SVs>],[<BEIDOU-SVs>], <UNKNOWN>, [<lat>],    [<N/S>],[<log>],     [<E/W>],[<date>],[<UTC-time>],[<alt>], [<speed>],[<course>],[<PDOP>],[<HDOP>],[<VDOP>], <unknown>
    ...
    if (fixMode == 1 || fixMode == 2 || fixMode == 3) {
     ...
     ivsat = streamGetIntBefore(',');     // GPS satellite valid numbers     // GPS satellite valid numbers
      streamSkipUntil(',');               // GLONASS satellite valid numbers
      streamSkipUntil(',');               // BEIDOU satellite valid numbers
      streamSkipUntil(',');               // unknown
      ...
      if (speed != NULL) *speed = ispeed * 1.852; // convert from knots to kn/h
